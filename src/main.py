...
from .storage_router import find_duplicate_job, get_jobs, update_pipeline_result
...
def _evaluate_job(job, config: dict, sponsor_rows: list[dict], aliases: dict) -> tuple[SponsorResult, object, str, int]:
...
def reprocess_jobs(
    ...
    for index, (job_id, job) in enumerate(items, start=1):
        sponsor_result, filter_result, pipeline_status, final_score = _evaluate_job(job, config, sponsor_rows, aliases)
